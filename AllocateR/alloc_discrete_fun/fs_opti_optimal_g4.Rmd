...
title: |
  | Optimal Allocation
  | Conditional on Age (4 Age Groups), Income, Marital Status and Kids Count
...
2019 Age (4 Groups) Income, Kids Count and Martial Status (non-stochastic) based optimal linear allocation (Utilitarian). Will solve for feasible optimal allocation results given different simulation structures:
...
st_img_suffix <- paste0(st_file_type_withspouse_shock, '_', st_img_suf_age_ybin, '_round1_optimal_g4')
...
st_img_suffix <- paste0(st_file_type_withspouse_shock, '_', st_img_suf_age_ybin, '_round2_optimal_g4')
...
#### Graph Round Two Allocations By Themselves by Age
```{r}
# Generate Graphs
allocate_type_levels <- c(Actual = "actual", Optimal = "optimal")
st_img_suffix <- paste0(st_file_type_withspouse_shock, '_', st_img_suf_age_ybin, '_round2_optimal_g4')
...
# Generate Graphs
stg_subtitle_stack=paste0(slb_subtitle_stack_2nd, stg_subtitle)
st_img_suffix <- paste0(st_file_type_withspouse_shock, '_', st_img_suf_age_ybin, '_round2_optimal_g4_stack')
...
print(ls_pl_r2$mc)
```
#### Opti C, 2nd Optimal by itself

```{r}
print(ls_pl_2nd$checks_c)
```
...
#### Opti V, 2nd Optimal by itself

```{r}
print(ls_pl_2nd$checks_v)
```
...
write.csv(df_alloc_all,
  paste0(srt_csv_path, "df_alloc_all_optimal_g4.csv"),
  row.names = TRUE)
...
for (svr_group in ls_svr_groups) {
  ...
  write.csv(df_alloc_combine_group_mean,
    paste0(srt_csv_path, "df_alloc_",svr_group, "_optimal_g4.csv"),
    row.names = TRUE)
  ...
  write.csv(df_alloc_combine_group_mean_oneless,
    paste0(srt_csv_path, "df_alloc_",svr_group,"_without_optimal_g4.csv"),
    row.names = TRUE)
...
for (svr_group in ls_svr_groups) {
  ...
  # Export
  write.csv(df_alloc_combine_group_mean_oneless,
    paste0(srt_csv_path, "df_alloc_",svr_group,"_without_noage_optimal_g4.csv"),
    row.names = TRUE)
...
write.csv(tb_rho_rev_mass_v_c_tab,
  paste0(srt_csv_path, "rev_optimal_g4.csv"),
  row.names = TRUE)
```
