...
ls_st_file_suffix_single_main <- c('snwx_bushchck_moredense_a65zh266zs5_b1_xi0_manna_96')
...
ar_double_triple_alloc_set4 <- c(52)
...
if (it_run_collection == 1) {
  ...
  # Generate the full allocation queue for the benchmark result
  # For bush check the benchmark result is the stimulus result.
  ls_st_file_suffix <- c(ls_st_file_suffix_single_main)
  ls_bl_per_capita <- c(TRUE)
  ar_rho <- c(1)
          ...
          for (it_age_type in c(1)) {
            ...
            for (it_solu_round in c(1)) {
              ...
              if (it_solu_group >= 101 & it_solu_group <=105){
                ...
                it_max_checks <- 72
                spt_img_save_use <- paste0(spt_img_save, '_7200_', st_img_suf_age_ybin,'/')
                srt_csv_path_use <- paste0(srt_csv_path, '_7200_', st_img_suf_age_ybin,'/')
                st_solu_name_check_count <- '72 checks'
                st_image_name <- '_7200'
              } else if (it_solu_group >= 111 & it_solu_group <=115) {
                # x05 to x07 is allocating up to 200k, common bounds for all
                it_max_checks <- 96
                spt_img_save_use <- paste0(spt_img_save, '_9600_', st_img_suf_age_ybin,'/')
                srt_csv_path_use <- paste0(srt_csv_path, '_9600_', st_img_suf_age_ybin,'/')
                st_solu_name_check_count <- '96 checks'
                st_image_name <- '_9600'
                ...
                it_check_headorspouse <- 6
                it_check_perkids <- 3
                spt_img_save_use <- paste0(spt_img_save, '_6ca3ck_', st_img_suf_age_ybin,'/')
                srt_csv_path_use <- paste0(srt_csv_path, '_6ca3ck_', st_img_suf_age_ybin,'/')
                st_solu_name_check_count <- 'actual checks adults'
                st_image_name <- '_6ca3ck'
                ...
                it_check_headorspouse <- 6
                it_check_perkids <- 6
                spt_img_save_use <- paste0(spt_img_save, '_6ckid_', st_img_suf_age_ybin,'/')
                srt_csv_path_use <- paste0(srt_csv_path, '_6ckid_', st_img_suf_age_ybin,'/')
                st_solu_name_check_count <- '6 checks adult 6 checks kids'
                st_image_name <- '_6ckid'
              } else if (it_solu_group >= 141 & it_solu_group <=145) {
                # adult 18 checks
                # four kids married = 9*2 + 3*4
                bl_threshold <- TRUE
                it_max_checks <- 30
                it_check_headorspouse <- 9
                it_check_perkids <- 3
                spt_img_save_use <- paste0(spt_img_save, '_9cadt_', st_img_suf_age_ybin,'/')
                srt_csv_path_use <- paste0(srt_csv_path, '_9cadt_', st_img_suf_age_ybin,'/')
                st_solu_name_check_count <- '9 checks adult 3 checks kids'
                st_image_name <- '_9cadt'
              } else if (it_solu_group >= 151 & it_solu_group <=155) {
                # double adult allocation from 12 to 24, kids 5 to 10
                # four kids married = 9*2 + 6*4
                bl_threshold <- TRUE
                it_max_checks <- 42
                it_check_headorspouse <- 9
                it_check_perkids <- 6
                spt_img_save_use <- paste0(spt_img_save, '_9ca6ck_', st_img_suf_age_ybin,'/')
                srt_csv_path_use <- paste0(srt_csv_path, '_9ca6ck_', st_img_suf_age_ybin,'/')
                st_solu_name_check_count <- '9 checks adult 6 checks kids'
                st_image_name <- '_9ca6ck'
                ...
                it_check_headorspouse <- 6
                it_check_perkids <- 9
                spt_img_save_use <- paste0(spt_img_save, '_9ckid_', st_img_suf_age_ybin,'/')
                srt_csv_path_use <- paste0(srt_csv_path, '_9ckid_', st_img_suf_age_ybin,'/')
                st_solu_name_check_count <- '6 checks adult 9 checks kids'
                st_image_name <- '_9ckid'
                ...
                it_check_perkids <- 3
                spt_img_save_use <- paste0(spt_img_save, '_12cadt_', st_img_suf_age_ybin,'/')
                srt_csv_path_use <- paste0(srt_csv_path, '_12cadt_', st_img_suf_age_ybin,'/')
                st_solu_name_check_count <- '12 checks adult 3 checks kids'
                st_image_name <- '_12cadt'
              } else if (it_solu_group >= 181 & it_solu_group <=185) {
                # triple adult allocation from 12 to 36, kids 5 to 15
                # four kids married = 12*2 + 9*4 = 132
                bl_threshold <- TRUE
                it_max_checks <- 60
                it_check_headorspouse <- 12
                it_check_perkids <- 9
                spt_img_save_use <- paste0(spt_img_save, '_12ca9ck_', st_img_suf_age_ybin,'/')
                srt_csv_path_use <- paste0(srt_csv_path, '_12ca9ck_', st_img_suf_age_ybin,'/')
                st_solu_name_check_count <- '12 checks adult 9 checks kids'
                st_image_name <- '_12ca9ck'
              }
              ...
              if (it_solu_group >= 101 & it_solu_group <= 199) {
                ...
                ls_stimulus_specs <- list(st_biden_or_trump = 'bushchck',
                                          fl_stimulus_child=it_check_perkids*fl_percheck_dollar,
                                          fl_stimulus_adult_min=3*fl_percheck_dollar,
                                          fl_stimulus_adult_max=it_check_headorspouse*fl_percheck_dollar,
