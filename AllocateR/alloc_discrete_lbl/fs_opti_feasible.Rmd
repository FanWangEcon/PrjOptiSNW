...
# st_file_type <- 'moredense_a65zh266zs5_e2m2_b1'
...
#### Adjustment Function Flattening

This is the preferred method, the next unit of allocation's marginal effects is foreced to be not higher than the previous unit: 
...
#### Adjustment Function Cumusum

This method pushes marginal value changes too much lower, uses a cumulative sum. This should not be used.
...
### Alternative Adjustment Methods and Visualizae Corrections

Which are the top cells that requires adjustments, what do they look like prior and post adjustments?

#### Alternative C Adjustments Outputs
...
df_input_il_noninc_test <- df_input_il %>%
  arrange((D_il)) %>%
  group_by(id_i) %>%
...
df_input_il_noninc_test <- df_input_il_noninc_test %>%
  arrange((D_il)) %>%
  group_by(id_i) %>%
  ...
  group_by(id_i) %>%
  mutate(D_il = row_number()) %>%
  left_join(df_input_il_noninc_test, by=(c('id_i'='id_i', 'D_il'='D_il'))) %>%
  mutate(c_alpha_il_gap = c_alpha_il - c_alpha_il_noninc_flatten) %>%
  mutate(c_alpha_il_ratio = (c_alpha_il - c_alpha_il_noninc_flatten)/c_alpha_il) %>%
  ungroup()
```

#### Visualize Largest Adjustments Cases
...
REconTools::ff_summ_percentiles(df_input_il_noninc_test, FALSE)
...
ar_id_large_changes <- unique(df_input_il_noninc_test %>% filter(c_alpha_il_ratio > 0.001 & c_alpha_il_ratio < 0.002) %>% pull(id_i))
print(ar_id_large_changes)
# graph the top 10 cases
ar_id_large_changes <- ar_id_large_changes[1:5]

## C. Graph Some of the largest Changes Cases
# dev.off()
for (it_id in ar_id_large_changes) {

  df_id_i_1 <- df_input_il_noninc_test %>% filter(id_i == it_id)
...
print(mass_sum)
print(mass_sum/sum((df_plan_v_tilde_ygrpjm %>% filter(checks == 0))$mass))
...
Extract "Value" in this case optimal level of consumptiton reached when budget is spent optimally. 
...
## Allocation Results Cross-Sectional Analysis
...
## Allocation Results Full Queue Analysis and View 

Check various things:

```{r}
# 1. total probability been consdiered (note no older)
sum((df_plan_v_tilde_ygrpjm %>% filter(checks == 0))$mass)
# 2. Weighted aggregate C
df_plan_v_tilde_ygrpjm %>% filter(checks == 0) %>% summarise(c_agg = sum(ctilde*mass)/sum(mass))
# 3. di_il frame weighted aggregate c
df_il %>% filter(D_il == 1) %>% left_join(df_id %>% select(id_i, mass), by='id_i') %>% ungroup() %>%summarise(c_agg_df_il = sum(ctilde*mass)/sum(mass))
# 4. df_il_U weighted aggregate c
df_il_U %>% filter(D_il == 1) %>% left_join(df_id %>% select(id_i, mass), by='id_i') %>% ungroup() %>%summarise(c_agg_df_il_U = sum(c_A_il*mass)/sum(mass))
# 4. df_input_il weighted aggregate c
df_input_il %>% filter(D_il == 1) %>% left_join(df_id %>% select(id_i, mass), by='id_i') %>% ungroup() %>%summarise(c_agg_df_il_U = sum(c_A_il*mass)/sum(mass))
# 5. Adjusted Cs.
df_input_il_noninc_test %>% filter(D_il == 1) %>% left_join(df_id %>% select(id_i, mass), by='id_i') %>% ungroup() %>%summarise(c_agg_noninc = sum(c_A_il*mass)/sum(mass))
df_input_il_noninc %>% filter(D_il == 1) %>% left_join(df_id %>% select(id_i, mass), by='id_i') %>% ungroup() %>%summarise(c_agg_noninc = sum(c_A_il*mass)/sum(mass))
# 6. Input for Allocation
df_input_il_c %>% filter(D_il == 1) %>% left_join(df_id %>% select(id_i, mass), by='id_i') %>% ungroup() %>%summarise(c_agg_input_il_c = sum(A_il*mass)/sum(mass))
# 7. Allocation output
df_queue_il_long_c
```

For the C utilitarian, the aggregate outcome variable shows total consumption along the optimal allocation queue. 

Plot along the x-axis total resource available, the y-axis aggregate consumption under optimal allocation with the specific constraints here. 

Plot a vertical line, at the actual resource available level. Plot also a horizontal line, at the aggregate consumption achieved by the actual policy (Which corresponds to the REV point basically)


```{r}
# dataframe with mass and aggregate outcome for the utilitarian
df_queue_agg_c <- df_queue_il_long_c %>% filter(rho == ar_rho[1]) %>% select(V_star_Q_il)
df_queue_agg_mass_c <- df_queue_il_long_mass_c %>% filter(rho == ar_rho[1]) %>% select(mass_cumu_il, V_star_Q_il)
# starting aggregate outcome
(df_queue_agg_c$V_star_Q_il)[1]
(df_queue_agg_mass_c$V_star_Q_il)[1]
```
```{r}
df_queue_agg_mass_c_perc <- df_queue_agg_mass_c %>% 
  filter(!is.na(V_star_Q_il)) %>% ungroup() %>% 
  mutate(V_star_Q_il_inc_perc = 100*((V_star_Q_il-min(V_star_Q_il))/V_star_Q_il))
```

```{r}
fl_rev_x <- (1-as.numeric(tb_rho_rev_mass_c[1,] %>% pull(REV)))*as.numeric(mass_sum_covid_vox_actual)
c_perc_inc_actual <- max(df_queue_agg_mass_c_perc %>% 
      filter( mass_cumu_il < fl_rev_x) %>%
      pull(V_star_Q_il_inc_perc))
c_per_inc_opti <- max(df_queue_agg_mass_c_perc %>% filter(!is.na(V_star_Q_il)) %>% pull(V_star_Q_il_inc_perc))
```

```{r}
df_queue_agg_mass_c_perc %>% 
  ggplot(aes(x=mass_cumu_il*100, y=V_star_Q_il_inc_perc)) +
  geom_line(size=2) +
  geom_vline(xintercept=as.numeric(mass_sum_covid_vox_actual)*100, 
             linetype='dotted', size = 4, color = "blue") +
  geom_vline(xintercept=as.numeric(fl_rev_x)*100, 
             linetype='dashed', size = 4, color = "blue") +
  geom_hline(yintercept=as.numeric(c_perc_inc_actual), 
             linetype='dotted', size = 4, color = "red") +
  geom_hline(yintercept=as.numeric(c_per_inc_opti), 
             linetype='dotdash', size = 4, color = "red") +
  labs(title = 'Percent Increase in Consumption and Total Resources/Budget along Optimal Allocation Queue',
       subtitle = paste0('BLACK SOLID = The Optimal Allocation Queue for FEASIBLE ALLOCATION\n',
                         'RED DOT  = Percentage Increase in C achieved by Actual Policy\n',
                         'RED DASH DOT = Percentage Increase in C achieved by Optimal Policy under Actual Total Budget\n',
                         'BLUE DOT = Actual Total Budget \n',
                         'BLUE DASH = Total Budget Required to Achieve same Percentage increase in C as achieved by Actual Policy if allocations were optimal'),
       x = 'Average welfare check amount (18-64 year-olds), Budget = AvgAmount x Pop18to64',
       y = 'Aggregate Consumption Percentage Increase',
       caption = 'If the Black Line is Linear, that means MPC identical for all, No gain to Optimal Allocation\nEach Allocation Constraint produces its own black line')
```
