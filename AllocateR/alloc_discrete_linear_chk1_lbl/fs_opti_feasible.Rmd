---
title: "Linear Preference, Feasible Optimal Allocation, Allocate by Y, Kids, Marry, Consider <= 65"
output:
  html_notebook:
    toc: yes
---
...
df_input_ib_c <- df_input_il_covid_actual %>%
  ...
  mutate(alpha_o_i = case_when(actual_checks == 0 ~ 0,
                               TRUE ~ c_alpha_il)) %>%
  select(id_i, A_i_l0, alpha_o_i, beta_i, actual_checks, mass) %>%
  rename(mass_i = mass)
...
### Graph Consumption Marginal Per Check
