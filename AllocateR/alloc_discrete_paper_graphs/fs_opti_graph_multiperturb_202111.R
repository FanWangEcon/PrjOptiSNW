...
snm_main_noperturb <- 'Results202111'
it_disturb <- 99999
    ...
    for (st_which_solu in ls_st_file_suffix) {
      ...
      for (st_bound_files in ls_st_bound_files) {
        for (fl_rho in ar_rho) {
          ...
          ls_output_noperturb <- fs_opti_support_202111(st_which_solu,
          ...
          if (st_v_or_c_opti == "vopti") {
            ...
            df_alloc_combined <- df_alloc_cur_optimal_nopertrub %>%
              select(-X, -optimal_v_1st) %>% ungroup()
          ...
          for (it_graph_types in ls_it_graph_types) {
            ...
            if (bl_save_img) {
              ...
              #     height = 216, units='mm',
              #     res = 300, pointsize=7)
              st_img_name_full <- paste0('alloc_perturb_', st_bound_files, '_perc_by_group_v', it_graph_types, '_', st_v_or_c_opti, '.png')
