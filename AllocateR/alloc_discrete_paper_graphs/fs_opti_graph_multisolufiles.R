...
ls_st_file_suffix <- c(ls_st_file_suffix_trumpchk,
                       ls_st_file_suffix_bidenchk,
                       ls_st_file_suffix_bchklock)
ls_st_file_suffix <- c(ls_st_file_suffix_bidenchk_mixturealter)
    ...
    for (fl_rho in ar_rho) {
      for (it_csvsolu_combo_type in ls_it_csvsolu_combo_type) {
        ...
        if (it_csvsolu_combo_type == 1) {
          ...
          ls_st_file_suffix_bidenchk_mixturealter <-
            c('snwx_bidenchk_moredense_a65zh266zs5_b1_xi0_manna_168_4o6',
              'snwx_bidenchk_moredense_a65zh266zs5_b1_xi0_manna_168',
              'snwx_bidenchk_moredense_a65zh266zs5_b1_xi0_manna_168_8o6')
        ...
        if (it_csvsolu_combo_type == 11) {
          ls_st_file_name_short <- c('bchknoui', 'bidenchk')
          ls_st_file_suffix_bidenchk_mixturealter <-
        ...
        ls_st_file_suffix <- ls_st_file_suffix_bidenchk_mixturealter
        ...
        st_img_name_full <- paste0(srt_csv_allocate_subfolder, st_save_suffix, '.png')
        ...
        st_lock <- "With lockdown (\u03f0=0.67)"
        st_nolock <- "With lockdown (\u03f0=1.00)"
        if (it_csvsolu_combo_type == 21) {
          ar_st_age_group_leg_labels <- c(st_lock, st_nolock)
          ar_st_colours <- c(st_util_color, st_inter_color)
          ar_it_shape_val <- c(it_util_shape, it_inter_shape)

        } else if (it_csvsolu_combo_type == 22) {
          ar_st_age_group_leg_labels <- c(st_actual, st_lock, st_nolock)
          ar_st_colours <- c(st_actual_color, st_util_color, st_inter_color)
          ar_it_shape_val <- c(it_actual_shape, it_util_shape, it_inter_shape)
          ...
          for (plt_cur in ls_plots) {
            if (bl_save_img) {
              ...
              srt_img_aggcsvsolu_save_subfolder <- file.path(srt_img_aggcsvsolu_save_root,
                                                             paste0(srt_csv_allocate_subfolder, st_save_suffix),
                                                             '/')
              ...
              st_img_name_full_subfigure <- paste0(srt_csv_allocate_subfolder,
                                                   st_save_suffix, '_subfig', it_fig_ctr, st_file_suffix, '.png')
