fs_opti_support_202103 <- function(st_which_solu='snwx_trumpchk_moredense_a65zh266zs5_b1_xi0_manna_168_married',
                                   bl_per_capita=FALSE, fl_rho=1) {
  #' Similar to fs_opti_support_202102, however, allow for additional parameters so that folder names are renamed
  #' depending on per-capita vs per-household allocation, and depepndig on the rho elasticity used.
  #'
  #' Addition change include how MPC files are stored, they are now stored in an aggregate folder under root where
  #' only the MPC files are stored.
  #'
  #' @description
  #' see fs_opti_support
  #'
  ...
  snm_main <- 'Results202103'
  # rho specific folder
  snm_full_main_folder <- file.path(snm_main, paste0(st_per_capita, st_rho))
  ...
  srt_simu_path <- file.path(srt_root, 'Output202102', '/')
