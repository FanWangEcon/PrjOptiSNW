...
For ARP = American Rescue Plan (i.e. the Biden Checks)

Adjustments from PrjOptiSNW\invoke\202102\snw_res_b1_manna_mixture.m:

In 2019, Ratio of average Social Seciurity benefits between college and
...
for it_solve_jedc_rr = ar_it_solve_jedc_rr
    ...
    if (it_solve_jedc_rr >= 1 && it_solve_jedc_rr <= 3)
        fl_ss_non_college = 0.220; % Average SS non-college 2005-2009 as a share of GDP per capita
        fl_ss_college = 0.266; % Average SS college 2005-2009 as a share of GDP per capita
...
%             st_param_group_base = 'default_tiny';
            ...
            if strcmp(st_biden_or_trump, 'bchknoui') || strcmp(st_biden_or_trump, 'bcklknou')
                ...
                % note since b=1, xi does not matter
                xi=0;
                b=1;
            end
            mp_params('xi') = xi;
