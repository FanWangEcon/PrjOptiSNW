%% SNW_A4CHK_UNEMP solves for Asset Position Corresponding to Check Level
...
if (~isempty(varargin))
    ...
    if (length(varargin)==3)
        [welf_checks, TR, xi, b, V_unemp] = varargin{:};
        mp_controls = snw_mp_control('default_base');
    elseif (length(varargin)==5)
        [welf_checks, TR, xi, b, V_unemp, mp_params, mp_controls] = varargin{:};
    end
