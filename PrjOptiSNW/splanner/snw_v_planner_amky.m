...
for it_ctr=1:size(a_eta_educ_in_inc_group_idx,1)
    ...
    for etap=1:n_etagrid
        for kidsp=1:n_kidsgrid
            condi_mass_etap_kidsp = condi_mass*pi_eta(eta,etap)*pi_kids(kids,kidsp,j,educ,married);
            V_planner=V_planner ...
                + vals(1)*condi_mass_etap_kidsp*EV(j+1,inds(1),etap,educ,married,kidsp,welf_checks+1)...
                + vals(2)*condi_mass_etap_kidsp*EV(j+1,inds(2),etap,educ,married,kidsp,welf_checks+1);
