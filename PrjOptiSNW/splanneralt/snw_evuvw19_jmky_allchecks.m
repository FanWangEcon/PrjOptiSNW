...
%    Call the SNW_EVUVW19_JMKY function to find the Value at all Checks increments.
...
%    individual check amount calculations.
%
...
%    structure.
%
%    * ST_BIDEN_OR_TRUMP string with values trumpchk or bidenchk or bushchck
%    * BL_LOAD_MAT boolean if true load saving mat file with VFI and
%    distributiona results if the file exists
%
%    This function was initiall designed to handle the COVID problem, the
%    revised version of the program handles both the 2007/8/9 Bush stimulus
%    check problem, and the 2019/20/21 Trump and Biden stimulus check
%    problems.
%
%    The key features of the Bush stimulus checks are: i) determined based
%    on 2007 information; ii) checks received in 2008, when the great
%    recession has not arrived yet, but all expect it to in 2009; iii) the
%    Great Recession hits in 2009, putting some people, based on education
%    and age, into unemployment state with a shared unemployment duration
%    and lost income and also UI benefits (calibrated to match overall UI
%    share of wages); iv) the economy returns to steady-state in 2010.
%
%    The key features of the Trump stimulus check are: i) determined based
%    on 2019 information; ii) checks received in 2020, ex-post the
%    realization of one-time MIT shock, the one-time MIT shock is
%    conditional on income and age groups; iii) state of the economy
%    returns to steady-state in 2021.
%
%    The key features of the Biden stimulus check are: i) determined based
%    on 2020 information; ii) checks received in 2021, ex-post the
%    realization of the second one-time MIT shock, which similar to the
%    first MIT shock, is conditional on income and age groups; iii) state
%    of the economy returns to steady-state in 2022; iv) trump checks were
%    provided in 2020, which changes saves and consumption and
%    distributions in 2020, and appropriate adjustments are made so that
%    the biden check is conditional on the distributional changes in
%    endogenous savings due to the Trump checks.
%
%    Details on the handling of 08 checks:
%    1. V: Solve steady state, equivalent to 2010
%    2. Solve for Value if Unemployed in 2009, Unemployed in 2009 same as steady 2010
%       - V^{U,09}, V^{E,09}=V
%    3. Household EV' in 2008, integrating over two employment states
%       - EV^{09;X} = P(U|X)*V^{U,09} + (1-P(U|X))*V^{E,09}
%    4. Solve for Value/Policy in 2008 given EV'
%       - V^{08} = U(C^{08}) + \beta*EV^{09;X}(A^{08,\prime})
%    5. Solve for differential values of checks in 08
%       - V^{08;CHK}, for various CHK increments
%       - snw_v08_jaeemk.m
%    6. Expectation in 2007 forward looking given 07 states into 08.
%       - EV^{07|age_{07}, kids_{07}, marry_{07}, income_{07}; CHK_{08}}
%       - Integrating over 08 state-space conditional on 07, for each check
%       - snw_evuvw19_jaeemk_foc.m: EV^{07}(JAEEMK) = int{V^{08}(J'A'E'E'M'K'|JAEEMK)}
%       - snw_evuvw19_jmky.m: EV^{07}(JMKY) = EV^{07}(JAEEMK|JMKY)
%       - snw_evuvw19_jmky_allchecks.m
%
...
%
%    See also SNWX_EVUVW19_JMKY_ALLCHECKS, SNWX_EVUVW07_JMKY_ALLCHECKS,
%    SNW_EVUVW19_JMKY, SNW_EVUVW19_JMKY_MASS, SNW_EVUVW19_JAEEMK_FOC,
...
if (~bl_load_mat || ~isfile(spt_mat_path))
    %% A. Solve VFI
    % ss = steady-state, either from 2010 onwards, or from 2022 onwards
    % ss_2020 interpreted also as ss_2009
...
%     end
    ...
    % call above, will be using the xi and b parameters provided via mp_params
    [V_unemp_2020,~,cons_unemp_2020] = snw_vfi_main_bisec_vec(mp_params, mp_controls, V_ss);

    % Bush checks solution, solve once for 2008 optimal choices
    if (strcmp(st_biden_or_trump, 'bushchck'))
        % V_unemp_2009 = V_unemp_2020
        % V_emp_2009 = V_ss_2020
        [V_2008, ap_2008, cons_2008, ~] = ...
            snw_v08p08_jaeemk(mp_params, mp_controls, V_ss_2020, V_unemp_2020);
    end
    ...
    % Alternatively, this is the 07 distribution for the Bush Checks.
    ...
    if (strcmp(st_biden_or_trump, 'trumpchk') || ...
        strcmp(st_biden_or_trump, 'bushchck'))
        % no actions needed
    ...
    %% C. Pre-compute  and store
    if (strcmp(st_biden_or_trump, 'bushchck'))

        % Pre-compute
        cl_st_precompute_list = {'a', ...
            'inc', 'spouse_inc', 'ref_earn_wageind_grid', ...
        ...
        % save
        save(spt_mat_path, 'Phi_true', 'ap_ss', 'V_2008', 'cons_2008',...
            'ar_a_amz', ...
            'ar_inc_amz', 'ar_spouse_inc_amz', 'ref_earn_wageind_grid', ...
            'inc_tot_ygroup_grid', 'ar_z_ctr_amz', ...
            'Phi_true_jmky');
...
%     mp_controls_parfor('bl_print_evuvw19_jmky_allchecks') = false;
    ...
    parfor i=1:n_welfchecksgrid
        ...
        % Run
        % evuvw19 equivalent for evuvw07
