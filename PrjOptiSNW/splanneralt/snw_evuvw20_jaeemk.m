...
if (strcmp(st_solu_type, 'matlab_minimizer'))
    [V_U,C_U]=snw_a4chk_unemp(...
        welf_checks, V_unemp_2020, cons_unemp_2020, mp_params, mp_controls);
    [V_W,C_W]=snw_a4chk_wrk(...
        welf_checks, V_ss_2020, cons_ss_2020, mp_params, mp_controls);
else
    % always use bisec_vec unless specified to use matlab_minimizer, no
    % grid_search option for this
    if exist('spt_mat_path','var')
        [V_W,C_W]=snw_a4chk_wrk_bisec_vec(welf_checks, mp_params, mp_controls, spt_mat_path);
        [V_U,C_U]=snw_a4chk_unemp_bisec_vec(welf_checks, mp_params, mp_controls, spt_mat_path);

    else
        [V_W,C_W]=snw_a4chk_wrk_bisec_vec( ...
            welf_checks, V_ss_2020, cons_ss_2020, mp_params, mp_controls, ...
            ar_a_amz, ar_inc_amz, ar_spouse_inc_amz);
        clear V_ss_2020 cons_ss_2020 ar_inc_amz ar_spouse_inc_amz

        [V_U,C_U]=snw_a4chk_unemp_bisec_vec( ...
            welf_checks, V_unemp_2020, cons_unemp_2020, mp_params, mp_controls, ...
            ar_a_amz, ar_inc_unemp_amz, ar_spouse_inc_unemp_amz);
        clear V_unemp_2020 cons_unemp_2020 ar_inc_unemp_amz ar_spouse_inc_unemp_amz
...
if exist('spt_mat_path','var')
    load(spt_mat_path, 'ref_earn_wageind_grid');
end
