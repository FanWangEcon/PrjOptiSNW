...
n_agrid=151; % 151; % 351; % No. of grid points for assets
n_eta_H_grid=9; % No. of grid points for persistent labor productivity shocks
n_eta_S_grid=1; % 5; % 1; % No. of grid points for spousal labor productivity shocks (=1 corresponds to deterministic spousal income)
...
%             toc;
            ...
            % Aggregation
            [Phi_true,Phi_adj,A_agg,Y_inc_agg,median_HH_income,it]=Aggregation(ap,cons,stat_distr_eta,stat_distr_educ,stat_distr_married,stat_distr_kids);
%              [Phi_true,Phi_adj,A_agg,Y_inc_agg,median_HH_income,it]=Aggregation_grid_search(ap,cons,stat_distr_eta,stat_distr_educ,stat_distr_married,stat_distr_kids);
...
disp('UPDATE TO COMPUTE LEVEL OF AGGREGATE WELFARE CHECKS AND GOVERNMENT CONSUMPTION')
disp('Need Gov_cons and Covid_checks')
% Covid_checks=sum(Phi_true(s)*W(s)) in model units. COMMENT: VERIFY THAT
% TOTAL WELFARE CHECKS AMOUNT TO ABOUT 1-3 PERCENT OF GDP
Gov_cons=Y_inc_agg*g_cons;
error('fix above!!')
...
a2_pre_COVID=a2; % a2 from pre-COVID model

a2_COVID=find_tax_rate(a2_guess,Phi_true,Gov_cons,Covid_checks,xi,b,cutoffs);

a2=a2_pre_COVID; % Maintain a2 from pre-COVID model since the model from 2021 is identical to the pre-COVID model
...
[V_working_tax,~,cons_working_tax,~]=VFI_working_tax(A_aux,B_aux,Aeq,Beq,nonlcon,options,V,a2_COVID);
disp('Compute policy functions when taxes adjust in the event of unemployment')
[V_unemp_tax,~,cons_unemp_tax,~]=VFI_unemp_tax(A_aux,B_aux,Aeq,Beq,nonlcon,options,V,xi,b,a2_COVID);
