function [V_U,exitflag_fsolve]=V_unemp_proxy(welf_checks,TR,xi,b,V_ss,cons_unemp,ap_unemp,options2)
           ...
           for educ=1:n_educgrid % Educational level
               for married=1:n_marriedgrid % Marital status
                   for kids=1:n_kidsgrid % Number of kids
                       ...
                       flow=vals(1)*utility(cons_unemp(j,inds(1),eta,educ,married,kids),married,kids)+vals(2)*utility(cons_unemp(j,inds(2),eta,educ,married,kids),married,kids);
                       
                       % Linear interpolation
                       ind_aux=find(agrid<=ap_unemp(j,inds(1),eta,educ,married,kids),1,'last');
                       
                       if ap_unemp(j,inds(1),eta,educ,married,kids)==0
                           indsp1(1)=1;
                           indsp1(2)=1;                       
                           valsp1(1)=1;
                           valsp1(2)=0;

                       elseif ap_unemp(j,inds(1),eta,educ,married,kids)>=agrid(n_agrid)
                           indsp1(1)=n_agrid;
                           indsp1(2)=n_agrid;                       
                           valsp1(1)=1;
                           valsp1(2)=0;
                           ...
                           indsp1(1)=ind_aux;
                           indsp1(2)=ind_aux+1;                       
                           valsp1(1)=1-((ap_unemp(j,inds(1),eta,educ,married,kids)-agrid(indsp1(1)))/(agrid(indsp1(2))-agrid(indsp1(1))));
                           valsp1(2)=1-valsp1(1);

                       end
                       
                       % Linear interpolation
                       ind_aux=find(agrid<=ap_unemp(j,inds(2),eta,educ,married,kids),1,'last');
                                              
                       if ap_unemp(j,inds(2),eta,educ,married,kids)==0
                           indsp2(1)=1;
                           indsp2(2)=1;                       
                           valsp2(1)=1;
                           valsp2(2)=0;

                       elseif ap_unemp(j,inds(2),eta,educ,married,kids)>=agrid(n_agrid)
                           indsp2(1)=n_agrid;
                           indsp2(2)=n_agrid;                       
                           valsp2(1)=1;
                           valsp2(2)=0;
                           ...
                           indsp2(1)=ind_aux;
                           indsp2(2)=ind_aux+1;                       
                           valsp2(1)=1-((ap_unemp(j,inds(2),eta,educ,married,kids)-agrid(indsp2(1)))/(agrid(indsp2(2))-agrid(indsp2(1))));
                           valsp2(2)=1-valsp2(1);
                       ...
                       V_U(j,a,eta,educ,married,kids)=flow+beta*psi(j)*(vals(1)*cont1+vals(2)*cont2);
