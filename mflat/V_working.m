function [V_VFI,exitflag_VFI]=V_working(welf_checks,TR,V_ss,A_aux,B_aux,Aeq,Beq,nonlcon,options)
               ...
               for married=1:n_marriedgrid % Marital status
                   for kids=1:n_kidsgrid % Number of kids
                       ...
                       if j==n_jgrid
                           ...
                           cons_VFI=(1+r)*agrid(a)+epsilon(j,educ)*theta*exp(eta_grid(eta))+SS(j,educ)+(married-1)*spouse_inc-max(0,Tax(inc,(married-1)*spouse_inc))+TR*welf_checks;
                           ...
                           amax=min(agrid(end),(1+r)*agrid(a)+epsilon(j,educ)*theta*exp(eta_grid(eta))+SS(j,educ)+(married-1)*spouse_inc-max(0,Tax(inc,(married-1)*spouse_inc))+TR*welf_checks );
                           
                           [ap_aux,~,exitflag_VFI(j,a,eta,educ,married,kids)]=fmincon(@(x)value_func_aux_working(x,j,a,eta,educ,married,kids,TR,welf_checks,V_ss),x0,A_aux,B_aux,Aeq,Beq,amin,amax,nonlcon,options);
...
%                            spouse_inc=spousal_income(j,educ,kids,( epsilon(j,educ)*theta*exp(eta_grid(eta)) )*(xi+b*(1-xi)),SS(j,educ));
                           ...
                           c_aux=(1+r)*agrid(a)+epsilon(j,educ)*theta*exp(eta_grid(eta))+SS(j,educ)+(married-1)*spouse_inc-max(0,Tax(inc,(married-1)*spouse_inc))+TR*welf_checks-ap_aux;
...
%                          spouse_inc=spousal_income(j,educ,kids,epsilon(j,educ)*theta*exp(eta_grid(eta)),SS(j,educ));
                           ...
                           c_aux3=(1+r)*agrid(a)+epsilon(j,educ)*theta*exp(eta_grid(eta))+SS(j,educ)+(married-1)*spouse_inc-max(0,Tax(inc,(married-1)*spouse_inc))+TR*welf_checks;
